...
import { toggleIsCouponModalVisible, isCouponModalVisibleOnScreen, setFetchedCoupons, fetchedCoupons } from "../store/slices/nonPersistSlice";
...
const CouponsModule = forwardRef((props, ref) => {
    ...
    const modalShowStatus = useSelector(isCouponModalVisibleOnScreen);
    const toggleModal = () => {
        dispatch(toggleIsCouponModalVisible(!modalShowStatus))
    ...
    const fetchCoupons = async () => {
        ...
        try {   
            ...
            const data = await response.json();
            dispatch(setFetchedCoupons(data));
            // setCouponsData(data);
                ...
                <div className="flex">
...

                            {
                                cartData.coupons.length ? (
                                    <span className="inline-block mt-1 text-xs text-[#4d4d4d] font-medium">
                                        You saved additional {formatCurrency(cartData.coupons[0].totals.total_discount, cartData.coupons[0].totals)}
                                    </span>
                                ) : (
                                <span className="inline-block mt-1 text-xs text-[#4d4d4d] font-medium">
...

                    {
                        cartData.coupons.length ? (
                            ...
                            <div className="offers flex text-sm text-primary font-medium items-center gap-1 cursor-pointer h-full leading-none">
