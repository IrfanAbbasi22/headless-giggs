        ...
        <div className="p-2 flex flex-col gap-2 w-full">
          ...
          <div className="productCardPricing flex justify-start items-center gap-2">
            {product.prices.price && (
              <p className="discountedPrice font-semibold">
                {formatCurrency(product.prices.price * product.quantity, cartData?.totals)}
              </p>
            )}
            {product.prices.regular_price !== product.prices.price && (
              <del className="originalPrice text-sm text-black opacity-50">
                {formatCurrency(product.prices.regular_price * product.quantity, cartData?.totals)}
              </del>
            )}
            {product.prices.regular_price !== product.prices.price && (
          ...
          {product.prices.regular_price !== product.prices.price && (
            <p className="text-green-500 text-sm font-medium">
              You saved {formatCurrency(product.prices.regular_price * product.quantity -
                product.prices.price * product.quantity, cartData?.totals)}
            </p>
          )}
