...
import { cartDetails, addToCart, updateQty, loadCartFromWoo, cartCurrency } from '../store/slices/cartSlice';
...
export default function ProductCard({ product }){
    ...
    const curCurrency = useSelector(cartCurrency);
    ...
    return (
        <div className="productCard flex flex-col" key={`productID-${product.id}`}>
            ...
            <div className="productCardInfo mt-4">
                <Link key={product.slug} href={product.slug} className="productCardTitle line-clamp-2 hover:text-primary transition-all">
                    { product.name }
                </Link>
                
                <div className="productCardPricing flex justify-start items-center gap-2 mt-2 mb-4">
                    {product.price && 
                        <p className="discountedPrice font-semibold"> {curCurrency}{product.price} </p>
                    }
                    {product.on_sale && 
                        <del className="originalPrice text-sm text-black opacity-50">{curCurrency}{product.regular_price}</del>
                    }
                    {product.on_sale &&
                ...
                <div className="productCardATC flex justify-center">
                    {isAdded ? (
                        <div className="quantityControls flex items-center justify-center gap-2 border border-primary rounded relative disabled:pointer-events-none disabled:opacity-50" disabled={loading}>
                            <button onClick={() => handleQtyChange(-1)} 
                                disabled={cartItem?.quantity <= 0 || loading}
                                className="quantityBtn text-primary text-sm hover:bg-[#146EB41A] py-1 px-2 disabled:opacity-50">-</button>
                            ...
                            {loading ? (
                                <svg className="animate-spin h-5 w-5 text-primary absolute" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                        ...
                        <button onClick={() => handleAddToCart(quantity)} disabled={loading || isAdded || product.stock_status != "instock"} 
                            className={`text-sm transition-all uppercase border bg-opacity-10 border-primary hover:bg-[#146EB41A] text-primary font-bold py-1 px-5 rounded flex items-center justify-center relative`}>
                            {loading ? (
                                <svg className="animate-spin h-5 w-5 text-primary absolute" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                                    <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
